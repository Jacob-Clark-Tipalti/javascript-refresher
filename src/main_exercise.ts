...
function newToPrintForMultipleOf3(prevToPrint: string): string {
    return prevToPrint + 'Fizz';
}

function newToPrintForMultipleOf5(prevToPrint: string): string {
    return prevToPrint + 'Buzz';
}

function newToPrintForMultipleOf7(prevToPrint: string): string {
    return prevToPrint + 'Bang';
}

function newToPrintForMultipleOf11(): string {
    return 'Fezz';
}

function newToPrintForMultipleOf13(prevToPrint: string): string {
    let indexOfB = prevToPrint.indexOf('B');
        if (indexOfB === -1) {
...
function newToPrintForMultipleOf17(toPrint: string): string {
    toPrint = toPrint.replace(/([A-Z])/g, ' $1').trim();
    let toPrintWords = toPrint.split(' ');
